...
async function run(): Promise<void> {
  ...
  try {
    ...
    await DataCaptureContext.forLicenseKeyInElectronPath(licenseDataPath, {
  ...
  // Let the SDK select the best camera. The camera is off by default and must be turned on to start streaming
  // frames to the data capture context for recognition.
  ...
  const barcodeCapture = await BarcodeCapture.forContext(DataCaptureContext.sharedInstance, settings);
