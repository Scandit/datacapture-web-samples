...
// Shared function to render document details (face, fields, barcode, viz data)
async function renderDocumentDetails(capturedId: SDCId.CapturedId, result: DocumentFragment): Promise<string> {
  let header = "";

  // Extract and show front review image if available
  const frontReviewImagePromise = capturedId.verificationResult.dataConsistency?.frontReviewImage();
  if (frontReviewImagePromise) {
    const frontReviewImage = await frontReviewImagePromise;
    if (frontReviewImage !== "" && frontReviewImage.length > 0) {
  ...
  // Show document-specific fields if available
...
export async function showDocumentResult(
  capturedId: SDCId.CapturedId,
  rejectionReason?: SDCId.RejectionReason,
  aamvaVerificationPromise?: Promise<AamvaBarcodeVerificationResult>
): Promise<void> {
  const result = document.createDocumentFragment();

  if (rejectionReason === SDCId.RejectionReason.DocumentExpired) {
    result.append(getPanel("warn", "Document is expired."));
  } else if (rejectionReason === SDCId.RejectionReason.InconsistentData) {
    result.append(getPanel("warn", "Information on front and back does not match."));
  } else {
    // No rejection reason - this is a valid document
    result.append(getPanel("passed", "Document is not expired."));
    result.append(getPanel("passed", "Information on front and back matches."));

    // AAMVA verification result is asynchronous, show timer and update later
    if (aamvaVerificationPromise) {
      result.append(
  ...
  // Render shared document details (includes front review image extraction)
  const header = await renderDocumentDetails(capturedId, result);
  ...
  // Handle asynchronous AAMVA verification for valid documents
  if (rejectionReason === undefined && aamvaVerificationPromise) {
    try {
      ...
      const timerPanel = elements.resultContent.querySelector(`[data-js-id="barcode-verification-result"]`);
      if (timerPanel) {
        timerPanel.replaceWith(barcodeVerificationResultBlock);
      }
    } catch (error: unknown) {
      console.log(error);
      const errorPanel = getPanel("warn", "No result for barcode verification.");
      const timerPanel = elements.resultContent.querySelector(`[data-js-id="barcode-verification-result"]`);
      if (timerPanel) {
        timerPanel.replaceWith(errorPanel);
      }
    }
  }
}
