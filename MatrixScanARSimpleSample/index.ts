...
async function run(): Promise<void> {
  ...
  // The barcode tracking process is configured through barcode ar settings,
  // they are then applied to the barcode ar instance that manages barcode recognition.
  ...
  // Create a new barcode ar mode with the settings from above.
  const barcodeAr = await BarcodeAr.forContext(context, settings);

  // Create the view
  const barcodeArView = await BarcodeArView.create(view, context, barcodeAr);

  // Create the highlight provider and configure an highlight for the barcode
  barcodeArView.highlightProvider = {
    // biome-ignore lint/suspicious/useAwait: <explanation>
    async highlightForBarcode(barcode: Barcode, callback: (highlight: BarcodeArHighlight) => void): Promise<void> {
  ...
  barcodeArView.annotationProvider = {
    // biome-ignore lint/suspicious/useAwait: <explanation>
    async annotationForBarcode(barcode: Barcode, callback: (highlight: BarcodeArAnnotation) => void): Promise<void> {
