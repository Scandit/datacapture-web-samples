...
class SDKProvider {
  ...
  private barcodeStatus: Map<`${Barcode["data"]}|${Barcode["symbology"]}`, "accepted" | "rejectable" | "rejected"> =
    new Map();
  ...
  configure({ licenseKey }: { licenseKey: string }): Promise<void> {
    ...
    this.configurePromise = (async () => {
      this.context = await DataCaptureContext.forLicenseKey(licenseKey, {
  ...
  private async createCloseUpAnnotation(barcode: Barcode): Promise<BarcodeArInfoAnnotation> {
    ...
    body1.text = "This is text in a large container. It can have multiple lines.";
      ...
      case Presets.Popovers:
        {
          ...
          this.barcodeArView.highlightProvider = {
            async highlightForBarcode(
              ...
              const existingStatus = self.barcodeStatus.get(barcodeKey);

              if (existingStatus) {
                // Apply the current status styling
                if (existingStatus === "rejectable") {
                ...
                if (isRejectable) {
                  ...
                  highlight.brush = new Brush(Color.fromRGBA(255, 0, 0, 1), Color.fromRGBA(255, 0, 0, 1), 1);
                  highlight.isPulsing = true;
                  self.barcodeStatus.set(barcodeKey, "rejectable");
                } else {
                  const checkMarkIcon = await new ScanditIconBuilder()
                    .withBackgroundShape(ScanditIconShape.Circle)
                    .withIcon(ScanditIconType.Checkmark)
                  ...
                  highlight.brush = new Brush(Color.fromRGBA(13, 133, 61, 1), Color.fromRGBA(13, 133, 61, 1), 1);
                  highlight.isPulsing = false;
                  self.barcodeStatus.set(barcodeKey, "accepted");
                }
              }
          ...
          this.barcodeArView.annotationProvider = {
            async annotationForBarcode(
              ...
              const barcodeKey = keyForBarcode(barcode);
              const barcodeStatus = self.barcodeStatus.get(barcodeKey);

              // Only create popover for rejectable barcodes that haven't been decided yet
              if (!barcodeStatus || barcodeStatus !== "rejectable") {
                return;
              }
              ...
              annotation.listener = {
                onPopoverButtonTapped: async (
                  ...
                  const currentStatus = self.barcodeStatus.get(popoverBarcodeKey);

                  if (!currentStatus || currentStatus !== "rejectable") {
                    return;
                  }

                  if (buttonIndex === 0) {
                    const highlight = self.barcodeArView?.getHighlightForBarcode(
                      popover.barcode
                    ) as BarcodeArCircleHighlight | null;
                    ...
                    // Reject button
                    highlight.brush = new Brush(Color.fromRGBA(255, 0, 0, 1), Color.fromRGBA(255, 0, 0, 1), 1);
                    highlight.isPulsing = false; // Stop pulsing
                    self.barcodeStatus.set(popoverBarcodeKey, "rejected");
                    ...
                    highlight.icon = await xMarkIconBuilder.build();
                    ...
                    highlight.brush = new Brush(Color.fromRGBA(13, 133, 61, 1), Color.fromRGBA(13, 133, 61, 1), 1);
                    highlight.isPulsing = false; // Stop pulsing
                    self.barcodeStatus.set(popoverBarcodeKey, "accepted");
                    popover.annotationTrigger = BarcodeArAnnotationTrigger.BarcodeScan;
                    highlight.icon = await checkMarkIconBuilder.build();
                  }
                },
              };
              callback(annotation);
